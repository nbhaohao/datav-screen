...
export default {
  ...
  setup(props) {
    ...
    watch(
      () => [props.todayUser, props.growthLastDay, props.growthLastMonth],
      (_, [prevTodayUser, prevGrowthLastDay, prevGrowthLastMonth]) => {
        startVal.value = prevTodayUser;
        startPercent.value = prevGrowthLastDay;
        startPercent2.value = prevGrowthLastMonth;
      }
    );
    return {
      startVal,
